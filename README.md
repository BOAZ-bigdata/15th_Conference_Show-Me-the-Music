...
## NLP 모델링
### - 데이터 크롤링 및 전처리
우선, 데이터 선정을 위해 브런치, 인스타그램 크롤링과 AI HUB 데이터 등을 확보하였고, 팀원끼리 분배해 데이터 라벨링을 진행하였다. 그 후, 데이터 전체 혹은 데이터별로 기본 KoBERT 모델을 돌렸고, 그 결과 AI HUB 데이터가 가장 본 팀의 프로젝트 목표와 적합하다 판단했고 이를 최종 데이터셋으로 선정하였다.
...
- KcBERT-large 라는 모델의 성능이 가장 좋았다. 이는 본 프로젝트가 (1)사용자의 단발적인 일기, 구어체 등의 비공식적인 글을 입력받는다고 전제하기에 (2)ai hub 의 말뭉치, 단발성 대화를 학습데이터로 사용했으며,(3) 고로 이와 방향성이 유사한 KcBert-large 모델의 성능이 가장 우수함을 시사한다. 하지만 추후에 분류 성능을 더 높이기 위해 모델별로 다시 하이퍼파라미터 튜닝과 추가적인 앙상블 등 다양한 기법을 적용할 것이다.
